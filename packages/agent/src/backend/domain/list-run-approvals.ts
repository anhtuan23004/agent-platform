...
/**
 * All approvals (pending AND decided) for a specific workflow run, ordered
 * by creation time ascending.  Powers historical step review on the PMO
 * ingestion canvas: completed steps render decided approval cards in
 * read-only mode so operators can review past mapping / normalization /
 * publish decisions without losing context.
 *
 * Only returns approvals addressed to the calling user (same RBAC boundary
 * as listMyPendingApprovals and listThreadApprovals).
 */
export async function listRunApprovals(opts: {
  session: SessionLike;
  runId: string;
  ...
  const result = await db.execute(sql`
    SELECT approval_id, run_id, step_id, proposed_payload,
     ...
     WHERE run_id = ${opts.runId}::uuid
       AND approver_user_id = ${opts.session.user_id}
