...
<title>PMO Resource Allocation Report</title>
<style>${CSS}</style>
</head>
<body>
<header class="report-header">
  <div><p class="eyebrow">PMO RESOURCE ALLOCATION</p><h1>Idle &amp; Overbook Report</h1></div>
...
function renderSeveritySection(
  ...
  ${renderFindingGroup('Overbook', overbook, report.recommendations, memberMap)}
  ${renderFindingGroup('Idle', idle, report.recommendations, memberMap)}
</section>`;
}
...
function renderFindingGroup(
  ...
  return `<section class="finding-group"><h3>${title}</h3>${findings
    .map((finding) => renderFinding(finding, recommendations, memberMap))
    .join('')}</section>`;
}
...
function renderFinding(
  ...
  const groups = recommendations.filter((group) => group.sourceMemberId === finding.memberId);
  ...
  ${finding.issueType === 'overbook' ? renderRecommendations(groups, memberMap) : ''}
</article>`;
}
...
function renderRecommendations(
  ...
  if (groups.length === 0) {
    return '<div class="recommendation-block"><h5>Rebalance recommendations</h5><p class="empty-state">No valid rebalance found</p></div>';
  }
  return `<div class="recommendation-block"><h5>Rebalance recommendations</h5>${groups
    .map((group) => {
      ...
      if (group.status === 'no_valid_rebalance_found') {
        return `${status}${warning}<p class="empty-state">No valid rebalance found: ${escapeHtml(group.noResultReasons.map(humanize).join(', ') || 'No candidate passed hard filters')}</p>`;
      }
...
.context-notes, .suggested-action, .suggested-actions, .recommendation-block { margin-top: 10px; padding: 9px; border-radius: 4px; background: #edf2f7; }
