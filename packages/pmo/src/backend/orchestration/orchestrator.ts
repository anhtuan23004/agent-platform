...
function agenticInstructionsText(): string {
  return [
    ...
    '',
    'MEMORY TOOLS:',
    '- pmo_loadContext: Load your persisted task state, recent session history, and thread-linked sessions at the start of each turn',
    ...
    '   - If hasState is false but threadSessions is non-null, the user uploaded',
    '     workbook(s) via the chat composer. Pick the most recent uploaded session',
    '     from threadSessions and start the ingestion workflow with it.',
    '   - If hasState is false AND threadSessions is null, no workbook has been',
    '     uploaded to this thread yet. If the user asked to ingest, prompt them to',
    '     attach a workbook using the composer paperclip button.',
    '',
    ...
    '     with the structured UI in the chat review card. Do NOT ask for structured',
    ...
    '   - When listing members, always include each member fullName (or memberId if missing),',
    '     role/department when present, and explanation.summary from tool output.',
    '   - Do not reply with member IDs alone when names are available in tool output.',
    '',
    ...
    '',
    'pmo_loadContext returns:',
    '- threadSessions: sessions linked to THIS chat thread (uploaded via composer). Use the',
    '  most recent one as the ingest target when the user asks to ingest.',
    '- recentSessions: the last few sessions for the tenant (any thread).',
    '',
    'Use this to provide context:',
    '- If threadSessions has an uploaded session, use it for ingestion. No need to ask.',
